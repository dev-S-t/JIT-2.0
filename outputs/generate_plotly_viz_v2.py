...
def create_comparison_figure():
    ...
    # Main title removed - subtitles become main headings
    
    # Colors - lighter brighter versions (transparency stays same)
    GREEN = '#90EE90'   # Light bright green
    RED = '#FF8A8A'     # Light bright red/coral
    GREEN_ALPHA = 0.55
    RED_ALPHA = 0.75
    BLUE_LINE = '#29B6F6'
    BLUE_DASH = '#1565C0'
    PURPLE = '#BA68C8'  # Lighter purple
    
    # ========== TOP: JIT ONLY (heading inside plot) ==========
    ax1.text(0.5, 0.95, 'JIT Only — Prediction errors cause shortages', 
             transform=ax1.transAxes, fontsize=18, fontweight='bold', 
             color=HEADING_COLOR, ha='center', va='top')
    
    # Green fill under actual demand
    ax1.fill_between(days, 0, actual, color=GREEN, alpha=GREEN_ALPHA, label='Demand')
    
    # Red fill - shortage gap
    ax1.fill_between(days, predicted, actual, 
                     where=(actual > predicted),
                     color=RED, alpha=RED_ALPHA, label='Shortage Risk',
                     interpolate=True)
    ...
    ax1.legend(loc='upper right', bbox_to_anchor=(0.98, 1.0), frameon=False, fontsize=10)  # Slightly inward from edge
    ax1.grid(True, linestyle=':', alpha=0.4, linewidth=0.5)  # Dotted grid both axes
    ax1.tick_params(colors='#555555')
    ax1.set_xlim(40, 70)
    
    # ========== BOTTOM: JIT + MICRO (heading inside plot) ==========
    ax2.text(0.5, 0.95, 'JIT + Micro-Expiry — Extensions reduce shortage', 
    ...
    # Green fill under actual demand
    ax2.fill_between(days, 0, actual, color=GREEN, alpha=GREEN_ALPHA)
    ...
    for ext_day in extension_days:
        ...
        ax2.bar(bar_pos, bar_height, width=bar_width, bottom=0,
                color=PURPLE, alpha=0.85, edgecolor=PURPLE, linewidth=2, zorder=10)
        # Add unit label on top
        ax2.text(bar_pos, bar_height + 1, f'{units}', 
                 ha='center', va='bottom', fontsize=11, fontweight='bold', color=PURPLE, zorder=11)
    ...
    ax2.legend(handles=[extension_patch], loc='upper right', bbox_to_anchor=(0.98, 1.0), frameon=False, fontsize=10)  # Slightly inward
    
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_PATH, 'figure9_jit_comparison_v2.png'), dpi=200, bbox_inches='tight')
    plt.savefig(os.path.join(OUTPUT_PATH, 'figure9_jit_comparison_v2.pdf'), bbox_inches='tight')
    plt.close()
    
    print("✓ Saved: figure9_jit_comparison_v2.png/pdf")
